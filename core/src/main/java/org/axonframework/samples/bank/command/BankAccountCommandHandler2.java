...
public class BankAccountCommandHandler2 {

	private Repository<BankAccount> repository;
	private EventBus eventBus;

	public BankAccountCommandHandler2(Repository<BankAccount> repository, EventBus eventBus) {
	...
	public void handle(BankAccountCreateCommand command) throws Exception {
		repository.newInstance(new Callable<BankAccount>() {

			@Override
			public BankAccount call() throws Exception {
				return new BankAccount(command.getBankAccountId(), command.getOverdraftLimit());
			}
		});
	}
	...
	public void handle(BankAccountMoneyDepositCommand command) {
		...
		bankAccountAggregate.execute(new Consumer<BankAccount>() {
			@Override
			public void accept(BankAccount bankAccount) {
				bankAccount.deposit(command.getAmountOfMoney());
			}
		});
	}
	...
	public void handle(BankAccountWithdrawMoneyCommand command) {
		...
		bankAccountAggregate.execute(new Consumer<BankAccount>() {
			@Override
			public void accept(BankAccount bankAccount) {
				bankAccount.withdraw(command.getAmountOfMoney());
			}
		});
	}
	...
	public void handle(BankTransferSourceDebitCommand command) {
		try {
			...
			bankAccountAggregate.execute(new Consumer<BankAccount>() {
				@Override
				public void accept(BankAccount bankAccount) {
					bankAccount.debit(command.getAmount(), command.getBankTransferId());
				}
			});
	...
	public void handle(BankTransferDestinationCreditCommand command) {
		try {
			...
			bankAccountAggregate.execute(new Consumer<BankAccount>() {
				@Override
				public void accept(BankAccount bankAccount) {
					bankAccount.credit(command.getAmount(), command.getBankTransferId());
				}
			});

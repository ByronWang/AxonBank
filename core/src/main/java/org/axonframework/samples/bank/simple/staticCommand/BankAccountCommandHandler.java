...
public class BankAccountCommandHandler {
	...
	public void handle(BankAccountCreateCommand command) throws Exception {
		repository.newInstance(() -> BankAccount.create(command.getBankAccountId(), command.getOverdraftLimit()));
	}
	...
	public void handle(BankAccountMoneyDepositCommand command) {
		...
		bankAccountAggregate.execute(bankAccount -> BankAccount.deposit(bankAccount, command.getAmountOfMoney()));
	}

	@CommandHandler
	public void handle(BankAccountWithdrawMoneyCommand command) {
		Aggregate<BankAccount> bankAccountAggregate = repository.load(command.getBankAccountId());
		bankAccountAggregate.execute(bankAccount -> BankAccount.withdraw(bankAccount, command.getAmountOfMoney()));
	}
	...
	public void handle(BankTransferSourceDebitCommand command) {
		try {
			Aggregate<BankAccount> bankAccountAggregate = repository.load(command.getBankAccountId());
			bankAccountAggregate.execute(bankAccount -> BankAccount.debit(bankAccount, command.getAmount()));
	...
	public void handle(BankTransferDestinationCreditCommand command) {
		try {
			Aggregate<BankAccount> bankAccountAggregate = repository.load(command.getBankAccountId());
			bankAccountAggregate.execute(bankAccount -> BankAccount.credit(bankAccount, command.getAmount()));
		} catch (AggregateNotFoundException exception) {
	...
	public void handle(BankTransferSourceReturnMoneyCommand command) {
		Aggregate<BankAccount> bankAccountAggregate = repository.load(command.getBankAccountId());
		bankAccountAggregate.execute(bankAccount -> BankAccount.returnMoney(bankAccount, command.getAmount()));
	}
}
